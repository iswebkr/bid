package kr.co.peopleinsoft.g2b.userInfo.scheduler.dminsttInfo;
...
import kr.co.peopleinsoft.g2b.userInfo.scheduler.dminsttInfo.job.CollectionLastFiveYearDataJob;
import kr.co.peopleinsoft.g2b.userInfo.scheduler.dminsttInfo.job.CollectionTodayAndYesterdayDataJob;
...
@Controller
@RequestMapping("/g2b/usrInfoService/dminsttInfoService/scheduler/UsrInfoService/dminsttInfoService")
...
public class DminsttInfoScheduler {
	...
	@Operation(summary = "최근 몇년간의 수요기관 정보 조회", description = "최근 몇년간의 수요기관 정보 조회")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionLastFiveYearDataJob", "dminsttInfoService"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "dminsttInfoService", "최근 몇년간의 수요기관 정보 조회", "0 0 22 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 수요기관 정보 조회", description = "이릍간의 수요기관 정보 조회")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "dminsttInfoService"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "dminsttInfoService", "이틀간의 수요기관 정보 조회", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}