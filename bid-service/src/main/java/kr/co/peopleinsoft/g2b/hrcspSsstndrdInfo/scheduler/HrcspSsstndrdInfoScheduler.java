...
@Controller
@RequestMapping("/g2b/hrcspSsstndrdInfoService/scheduler/HrcspSsstndrdInfoService")
...
public class HrcspSsstndrdInfoScheduler {
	...
	@Operation(summary = "최근 몇년간의 사전규격 데이터 수집", description = "최근 몇년간의 사전규격 데이터 수집")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionLastFiveYearDataJob", "HrcspSsstndrdInfoService"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "HrcspSsstndrdInfoService", "최근 몇년간의 사전규격 데이터 수집", "0 0 21 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 사전규격 데이터 수집", description = "이틀간의 사전규격 데이터 수집")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "HrcspSsstndrdInfoService"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "HrcspSsstndrdInfoService", "이틀간의 사전규격 데이터 수집", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}