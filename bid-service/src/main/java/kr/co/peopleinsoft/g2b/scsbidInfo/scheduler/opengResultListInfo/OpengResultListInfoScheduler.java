package kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultListInfo;
...
import kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultListInfo.job.CollectionLastFiveYearDataJob;
import kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultListInfo.job.CollectionTodayAndYesterdayDataJob;
...
@Controller
@RequestMapping("/g2b/scsbidInfoService/opengResultListInfo/scheduler/ScsbidInfoService/opengResultListInfo")
...
public class OpengResultListInfoScheduler {
	...
	@Operation(summary = "최근 몇년간의 개찰결과 데이터 수집", description = "최근 몇년간의 개찰결과 데이터 수집")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionLastFiveYearDataJob", "opengResultListInfo"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "opengResultListInfo", "최근 몇년간의 개찰결과 데이터 수집", "0 30 18 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 개찰결과 데이터 수집", description = "이틀간의 개찰결과 데이터 수집")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "opengResultListInfo"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "opengResultListInfo", "이틀간의 개찰결과 데이터 수집", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}