...
@Controller
@RequestMapping("/g2b/cntrctInfoService/scheduler/CntrctInfoService")
@Tag(
...
public class CntrctInfoScheduler extends CmmnAbstractController {
	...
	@Operation(summary = "최근 몇년간의 계약정보 데이터 수집", description = "최근 5년치 데이터 수집")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		...
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "CntrctInfoService", "최근 몇년간의 계약정보 데이터 수집", "0 0 19 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 계약정보 데이터 수집", description = "이틀간의 계약정보 데이터 수집")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "CntrctInfoService"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "CntrctInfoService", "이틀간의 계약정보 데이터 수집", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}