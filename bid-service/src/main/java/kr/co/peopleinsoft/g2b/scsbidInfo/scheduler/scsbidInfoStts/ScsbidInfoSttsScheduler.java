...
@Controller
@RequestMapping("/g2b/scsbidInfoService/scsbidInfoStts/scheduler/ScsbidInfoService/scsbidInfoStts")
...
public class ScsbidInfoSttsScheduler {
	...
	@Operation(summary = "최근 몇년간의 낙찰목록 현황 데이터 수집", description = "최근 몇년간의 낙찰목록 현황 데이터 수집")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionLastFiveYearDataJob", "scsbidInfoStts"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "scsbidInfoStts", "최근 몇년간의 낙찰목록 현황 데이터 수집", "0 30 18 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 낙찰목록 현황 데이터 수집", description = "이릍간의 낙찰목록 현황 데이터 수집")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "scsbidInfoStts"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "scsbidInfoStts", "이틀간의 낙찰목록 현황 데이터 수집", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}