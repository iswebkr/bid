...
@Controller
@RequestMapping("/scheduler/UsrInfoService/prcrmntCorpBasicInfoService")
...
public class PrcrmntCorpBasicInfoScheduler {
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		...
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "사용자정보", "사용자정보 - 조달업체 정보 조회", "0 0 21 * * ?", new HashMap<>());
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		...
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "사용자정보", "사용자정보 - 조달업체 정보 조회", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}