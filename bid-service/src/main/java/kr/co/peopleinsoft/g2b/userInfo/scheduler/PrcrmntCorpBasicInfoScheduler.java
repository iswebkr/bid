...
import kr.co.peopleinsoft.g2b.userInfo.scheduler.jobs.prcrmntCorpBasicInfo.ColctLatestPrcrmntCorpBasicInfoJob;
import kr.co.peopleinsoft.g2b.userInfo.scheduler.jobs.prcrmntCorpBasicInfo.PrcrmntCorpBasicInfoJob;
...
public class PrcrmntCorpBasicInfoScheduler {
	...
	@GetMapping("/prcrmntCorpBasicInfoJob")
	public ResponseEntity<String> prcrmntCorpBasicInfoJob(@RequestParam(required = false) String jobExpression) throws SchedulerException, JsonProcessingException {
		...
		cmmnScheduleManager.deleteJob("PrcrmntCorpBasicInfoJob", "사용자정보"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(PrcrmntCorpBasicInfoJob.class, "PrcrmntCorpBasicInfoJob", "사용자정보", "사용자정보 - 조달업체 정보 조회", cronJobExpression, new HashMap<>());
	...
	@Operation(summary = "조달업체정보 최신데이터 수집", description = "조달업체정보 최신데이터 수집", parameters = {
		@Parameter(name = "jobExpression", description = "Quartz 크론표현식 (ex : * 0 * * * ?) [초, 분, 시, 일, 월, 주, 년]", allowEmptyValue = true)
	})
	@GetMapping("/latest/colctLatestPrcrmntCorpBasicInfoJob")
	public ResponseEntity<String> colctLatestPrcrmntCorpBasicInfoJob(@RequestParam(required = false) String jobExpression) throws SchedulerException, JsonProcessingException {
		String cronJobExpression = StringUtils.defaultIfBlank(jobExpression, "0 0 * * * ?");
		cmmnScheduleManager.deleteJob("ColctLatestPrcrmntCorpBasicInfoJob", "최신자료수집"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(ColctLatestPrcrmntCorpBasicInfoJob.class, "ColctLatestPrcrmntCorpBasicInfoJob", "최신자료수집", "조달업체정보 최신데이터 수집", cronJobExpression, new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}