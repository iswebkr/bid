package kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultPreparPcDetail;
...
import kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultPreparPcDetail.job.CollectionLastFiveYearDataJob;
import kr.co.peopleinsoft.g2b.scsbidInfo.scheduler.opengResultPreparPcDetail.job.CollectionTodayAndYesterdayDataJob;
...
@Controller
@RequestMapping("/g2b/scsbidInfoService/opengResultPreparPcDetail/scheduler/ScsbidInfoService/opengResultPreparPcDetail")
...
public class OpengResultPreparPcDetailScheduler {
	...
	@Operation(summary = "최근 몇년간의 예비가격 상세 데이터 수집", description = "최근 몇년간의 예비가격 상세 데이터 수집")
	...
	public ResponseEntity<String> CollectionLastFiveYearDataJob() throws SchedulerException, JsonProcessingException {
		...
		cmmnScheduleManager.createCronJob(CollectionLastFiveYearDataJob.class, "CollectionLastFiveYearDataJob", "opengResultPreparPcDetail", "최근 몇년간의 예비가격 상세 데이터 수집", "0 30 18 * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}

	@Operation(summary = "이틀간의 예비가격 상세 데이터 수집", description = "이릍간의 예비가격 상세 데이터 수집")
	...
	public ResponseEntity<String> CollectionTodayAndYesterdayDataJob() throws SchedulerException, JsonProcessingException {
		cmmnScheduleManager.deleteJob("CollectionTodayAndYesterdayDataJob", "opengResultPreparPcDetail"); // 이전에 등록된 job 삭제
		cmmnScheduleManager.createCronJob(CollectionTodayAndYesterdayDataJob.class, "CollectionTodayAndYesterdayDataJob", "opengResultPreparPcDetail", "이틀간의 예비가격 상세 데이터 수집", "0 */10 * * * ?", new HashMap<>());
		String jobList = cmmnSchedulerInfoService.getAllJobsAndTriggersAsJson();
		return ResponseEntity.ok().body(jobList);
	}
}